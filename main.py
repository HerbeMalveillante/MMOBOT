...
extensions = ['cogs.profilecommands', 'cogs.databasecommands',
              'cogs.infocommands', 'cogs.step', 'cogs.misc', 'cogs.topgg']
...
async def get_pre(bot, message):
    ...
    prefixes += map("".join, itertools.product(*
                                               zip(config.prefix.upper(), config.prefix.lower())))
...
async def on_ready():
    ...
    # launching energy recovering loop from DatabaseCog cog
    bot.cogs["DatabaseCog"].energy.start()
...
async def ping(ctx):
    ...
    embed = discord.Embed(title="PONG :ping_pong: ", description="I'm online ! :signal_strength:",
                          colour=config.colour, timestamp=datetime.datetime.utcnow())
    embed.set_thumbnail(url=bot.user.avatar_url)
    embed.set_footer(text=bot.user.name + ' - requested by ' +
                     str(ctx.author), icon_url=ctx.author.avatar_url)
    message = await ctx.send(embed=embed)

    end = time.perf_counter()
    duration = (end - start) * 1000
    embed.add_field(name="Latency", value="{:.2f}ms".format(
...
async def activity():
    ...
    stats = [int(datetime.datetime.utcnow().timestamp()), len(bot.users), len(
        bot.guilds), database.get_player_number(), len(bot.cogs), len(bot.commands)]
