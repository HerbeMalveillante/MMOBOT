...
def getGames():
    ...
    # on utilise une methode de l'objet API qui nous retourne les jeux gratuts, et on navigue dans les dictionnaires pour récupérer les véritables jeux gratuits.
...
def parseGame(game):
    ...
    # keyImages est une liste d'au moins deux images.
    ...
    try:
        # ceci ne va marcher que sur le jeu actuellement gratuit.
        startDate = game['promotions']['promotionalOffers'][0]['promotionalOffers'][0]['startDate']
        # c'est pour ça qu'on le met dans un try.
        endDate = game['promotions']['promotionalOffers'][0]['promotionalOffers'][0]['endDate']
        # on en profite pour définir la variable pour voir si il est gratuit actuellement sur true
        currentlyFree = True
    except:
        try:
            # ceci ne marchera que sur un jeu bientôt gratuit.
            ...
            # un return met fin à l'execution du code de la fonction.
            return None
...
class MiscCommandsCog(commands.Cog):
    ...
    async def epic(self, ctx, arg="list"):
        ...
        if arg == "list":
            ...
            embed = discord.Embed(title=f"FREE GAMES ON THE EPIC GAME STORE",
                                  description=f"Use the command `{config.prefix}epic <{indexListString}>` to see the corresponding game.", colour=config.colour, timestamp=datetime.datetime.utcnow())
            embed.set_thumbnail(
            ...
            embed.add_field(name="Game index :", value="\n".join(
                [f"`{i} | {games[i]['name']}`" for i in range(len(games))]))
            ...
            embed = discord.Embed(title=f"FREE GAMES ON THE EPIC GAME STORE",
                                  description=f"Use the command `{config.prefix}epic list` to see all the available games.", colour=config.colour, timestamp=datetime.datetime.utcnow())
            if validurl.match(game['imageWide']) is not None:
                embed.set_thumbnail(url=game['imageWide'])
            else:
                embed.set_thumbnail(
                    url='https://worldofvoz.files.wordpress.com/2020/01/http-error-404-not-found.png')
            embed.set_footer(text=self.bot.user.name + ' - requested by ' +
                             str(ctx.author), icon_url=ctx.author.avatar_url)
            embed.add_field(name="Game name :", value=game['name'])
            embed.add_field(name="Game seller : ",
                            value=game['seller'], inline=False)
            value = "Currently Free" if game["currentlyFree"] else "Soon to be free"
            embed.add_field(
                name="Game state :", value=f"{value} (previously {game['originalPrice']})", inline=False)
            embed.add_field(name="Link to the game page :",
                            value=f"[Click me]({game['gameLink']})", inline=False)
            embed.add_field(name="Promotion dates :",
                            value=f"{game['startDate'][:10]} to {game['endDate'][:10]}", inline=False)
            if validurl.match(game['imageWide']) is not None:
                embed.add_field(
                    name="Images :", value=f"[Wide image]({game['imageWide']})\n[Tall image]({game['imageTall']})", inline=False)
