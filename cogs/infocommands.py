...
class InfoCog(commands.Cog):
    ...
    async def info(self, ctx):
        ...
        embed = discord.Embed(title=f"Informations about the bot :", description="",
    ...
    async def help(self, ctx):
        ...
        embed = discord.Embed(title=f"{self.bot.user.name} COMMAND LIST :",
                              description="Warning : this help command is temporary and will be replaced soon.", colour=config.colour, timestamp=datetime.datetime.utcnow())
        embed.set_thumbnail(url=self.bot.user.avatar_url)
        embed.set_footer(text=self.bot.user.name + ' - requested by ' +
                         str(ctx.author), icon_url=ctx.author.avatar_url)
    ...
    async def users(self, ctx):
        ...
        plt.ylabel('users')
        plt.title(
            f'Data updated every 10m. Dynamic data sampling : {len(usercount)}/{oldsamples} values')
    ...
    async def servers(self, ctx):
        ...
        plt.ylabel('servers')
        plt.title(
            f'Data updated every 10m. Dynamic data sampling : {len(servercount)}/{oldsamples} values')
